...
function getCountWeekendsInMonth(month, year) {
  ...
  const dates = new Array(countDay)
    .fill(1)
    .map((_, index) => new Date(year, month - 1, index + 1));

  let countWeekend = 0;
  dates.forEach((item) => {
    if (item.getDay() === 0 || item.getDay() === 6) countWeekend += 1;
  });
...
function getWeekNumberByDate(/* date */) {
  ...
  //   result = 52;
  // } else {
  //   result = Math.ceil((date - nextMonday) / (24 * 3600 * 1000) / 7);
  // }
  // return result;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (startDate <= endDate) {
    for (let i = 0; i < countWorkDays && startDate <= endDate; i += 1) {
      schedule.push(formatDateNew(startDate));
